...
export class AdminService {
  ...
  async getDashboardData() {
    ...
    const totalLands = await db.land.count();

    const architectureCompanies = await db.architectureCompany.count();
    const constructionCompanies = await db.constructionCompany.count();
    ...
    const latestProps = await db.property.findMany({
      take: 4,
      orderBy: { createdAt: 'desc' },
    });

    const latestLands = await db.land.findMany({
      take: 4,
      orderBy: { createdAt: 'desc' },
    });

    const pendingProps = await db.property.findMany({
      where: { status: 'Pending' },
      take: 5,
    });

    const pendingLands = await db.land.findMany({
      where: { status: 'Pending' },
      take: 5,
    });

    const totalCompanies =
      architectureCompanies + constructionCompanies;

    const monthNames = ['Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec', 'Jan'];

    const userGrowthData = monthNames.map((month, index) => ({
      month,
      count: index === monthNames.length - 1 ? totalUsers : 0,
    }));

    const propertyGrowthData = monthNames.map((month, index) => ({
      month,
      count: index === monthNames.length - 1 ? totalProperties : 0,
    }));

    const landGrowthData = monthNames.map((month, index) => ({
      month,
      count: index === monthNames.length - 1 ? totalLands : 0,
    }));
    ...
    return {
      kpiStats: [
        ...
        },
        {
          ...
          isPositive: totalProperties > 0,
        },
        {
          name: 'TOTAL LANDS',
          value: totalLands.toLocaleString(),
          change: totalLands > 0 ? `+${totalLands}` : '0%',
          isPositive: totalLands > 0,
        },
        {
  ...
  async getProperties() {
    ...
    const rejectedCount = props.filter(
  ...
  async getAnalyticsData() {
    ...
    const [totalProperties, totalLands] = await Promise.all([
      db.property.count(),
      db.land.count(),
    ]);
    ...
    return {
      ...
      trafficData: months.map((month) => ({
        month,
        pageViews: 0,
        uniqueVisitors: 0,
      })),

      conversionData: months.map((month) => ({
        month,
        leads: 0,
        inquiries: 0,
      })),
      ...
      propertyTypeBreakdown: [
        {
          ...
          percentage: totalProperties > 0 ? 50 : 0,
        },
        {
          type: 'Lands',
          count: totalLands,
          percentage: totalLands > 0 ? 50 : 0,
        },
      ],
    };
  }
