...
export const AgentLayout: React.FC<{ children?: React.ReactNode }> = ({ children }) => {
  ...
  const navItems = [
    ...
    { label: 'Browse Listings', href: '/property-listing', icon: '🏢' },
  ];

  const isActive = (href: string) => {
    if (href === '/dashboard') return location.pathname === '/dashboard' || location.pathname === '/dashboard/agent';
      ...
      <div className="flex-1 flex max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 gap-6">
        ...
        <aside className="hidden lg:flex flex-col w-64 bg-white rounded-2xl border border-gray-200/80 p-5 shrink-0 shadow-sm self-start sticky top-[88px]">
          ...
          <div className="flex items-center gap-3 pb-5 mb-5 border-b border-gray-100">
            <div className="size-11 rounded-full bg-[#345b79] text-white flex items-center justify-center font-bold text-lg">
            ...
            <div className="flex flex-col min-w-0">
              ...
              <span className="text-xs text-[#be5d3f] font-bold uppercase tracking-wider">Agent Account</span>
            </div>
          </div>
        ...
        <div className="lg:hidden flex items-center justify-between bg-white p-4 rounded-xl border border-gray-200 mb-2 w-full">
          <span className="text-xs font-bold text-[#345b79] uppercase tracking-wider">Agent Workspace</span>
          <button
            onClick={() => setMobileSidebarOpen(!mobileSidebarOpen)}
            className="text-xs bg-[#345b79] text-white font-bold px-3 py-1.5 rounded-lg"
          >
            {mobileSidebarOpen ? 'Close Menu' : 'Agent Menu'}
          </button>
        </div>
