    ...
    <div className="h-full overflow-y-auto p-[20px] md:p-[32px] space-y-[32px] bg-[#e6e0d4]">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-[24px]">
        <div className="bg-white border border-[#ccb7a3]/20 rounded-[12px] p-[24px] shadow-sm flex items-center gap-[20px]">
          ...
          <div>
            <h4 className="text-[30px] font-extrabold text-[#1d1d1d]">{totalUsersCount.toLocaleString()}</h4>
            <span className="text-[11px] font-bold text-[#6b879c] tracking-[0.6px] uppercase">Total Users</span>
          </div>
        </div>
        <div className="bg-white border border-[#ccb7a3]/20 rounded-[12px] p-[24px] shadow-sm flex items-center gap-[20px]">
          ...
          <div>
            <h4 className="text-[30px] font-extrabold text-[#1d1d1d]">{activeCount.toLocaleString()}</h4>
            <span className="text-[11px] font-bold text-[#6b879c] tracking-[0.6px] uppercase">Active Users</span>
          </div>
        </div>
        <div className="bg-white border border-[#ccb7a3]/20 rounded-[12px] p-[24px] shadow-sm flex items-center gap-[20px]">
          ...
          <div>
            <h4 className="text-[30px] font-extrabold text-[#1d1d1d]">{pendingCount}</h4>
            <span className="text-[11px] font-bold text-[#6b879c] tracking-[0.6px] uppercase">Pending Approval</span>
          </div>
        </div>
        <div className="bg-white border border-[#ccb7a3]/20 rounded-[12px] p-[24px] shadow-sm flex items-center gap-[20px]">
          ...
          <div>
            <h4 className="text-[30px] font-extrabold text-[#1d1d1d]">{suspendedCount}</h4>
            <span className="text-[11px] font-bold text-[#6b879c] tracking-[0.6px] uppercase">Suspended</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border border-[#ccb7a3]/20 rounded-[12px] shadow-sm overflow-hidden">
        ...
        <div className="p-[16px] md:p-[20px] border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-[16px]">
          ...
          <div className="text-[12px] font-bold text-[#6b879c] shrink-0">
            ...
            <tbody className="divide-y divide-gray-50 text-[13px]">
              {filteredUsers.map((user) => (
                <tr key={user.id} className="hover:bg-gray-50/40 transition-colors">
                  ...
                  <td className="py-[16px] px-[24px]">
                    <div className="flex items-center gap-[12px]">
                      ...
                      <div className="leading-tight">
                        <div className="font-bold text-[#1d1d1d]">{user.name}</div>
                        <div className="text-[10px] text-gray-400 font-semibold">ID: {user.id}</div>
                      </div>
                    </div>
                  </td>
                  ...
                  {/* Email */}
                  <td className="py-[16px] px-[24px] text-gray-600 font-bold">{user.email}</td>
                  ...
                  {/* Joined */}
                  <td className="py-[16px] px-[24px] text-gray-500 font-semibold">{user.joined}</td>

                  {/* Last Login */}
                  <td className="py-[16px] px-[24px] text-gray-500 font-semibold">{user.lastLogin}</td>
        ...
        <div className="p-[20px] border-t border-gray-100 flex items-center justify-between text-[13px] font-bold text-gray-500 bg-white">
          <button className="px-[12px] py-[6px] border border-[#ccb7a3]/40 rounded-[6px] hover:bg-gray-50 font-bold transition-all disabled:opacity-50" disabled>
            Previous
          </button>
          <div className="flex gap-[4px]">
            <button className="size-[32px] flex items-center justify-center rounded-[6px] bg-[#345b79] text-white font-bold">1</button>
            <button className="size-[32px] flex items-center justify-center rounded-[6px] hover:bg-gray-100">2</button>
            <button className="size-[32px] flex items-center justify-center rounded-[6px] hover:bg-gray-100">3</button>
          </div>
      ...
      {showAddUserModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-[16px] backdrop-blur-[2px]">
          <div className="w-full max-w-[420px] rounded-[12px] bg-white p-[24px] shadow-lg border border-[#ccb7a3]/30">
            <h4 className="text-[18px] font-extrabold text-[#1d1d1d] mb-[4px]">Add New User</h4>
            <p className="text-[12px] text-gray-400 mb-[20px] font-semibold">Create a new administrator, agent, or client profile.</p>
            
            <form onSubmit={handleAddUser} className="space-y-[16px]">
              <div>
                <label className="block text-[11px] font-bold text-[#6b879c] uppercase tracking-wider mb-[6px]">Full Name</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-[11px] font-bold text-[#6b879c] uppercase tracking-wider mb-[6px]">Email Address</label>
                <input
                  type="email"
                  required
              ...
              <div>
                <label className="block text-[11px] font-bold text-[#6b879c] uppercase tracking-wider mb-[6px]">System Role</label>
                <select
                  value={newUserRole}
